...
from docutils.nodes import Inline, TextElement, Node
...
def visit_titled_text_html(self: HTMLTranslator, node: titled_text) -> None:
    self.body.append('<span title="%s">' % self.attval(node.title))


def depart_titled_text_html(self: HTMLTranslator, _node: titled_text) -> None:
    ...
    def __call__(self, _name, raw_text: str, text: str,
        ...
        if not len(texts) == len(parts_without_suffixes):
            raise ExtensionError(
                "ROS element has an incompatible number of tokens. "
                "Expected %s, but got %s." % (parts_without_suffixes, texts)
            )
        ...
        titled.append(literal)
        titled.title = "ROS %s" % " from ".join(
            ["%s ""%s""" % (part, text)
